...
def load_config(dirname=None):
    ...
    print(f"loading config file from {dirname}.")
...
def timeit(func):
    ...
    def timeit_wrapper(*args, **kwargs):
        ...
        print(f'Function {func.__name__}{args} kwargs: {kwargs} Took {total_time:.4f} seconds')
        return result
    return timeit_wrapper

###########################################
# Raghav's stuff below here (go through it!)
def isfloat(num):
    try:
        float(num)
        return True
...
def get_origin_intersection(df1, df2):
    """
    ...
    df = gpd.sjoin_nearest(df1, df2, how='inner', distance_col=None)
...
def get_dest_intersection(df1, df2):
    """
    Intersect origin points of all linestrings with poylgons.
    ...
    df1['origin'] = df1.geometry.apply(lambda x: Point(x.coords[-1]))
    df1 = df1.set_geometry('origin')
    df = gpd.sjoin_nearest(df1, df2, how='inner', distance_col=None)
